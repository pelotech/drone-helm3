...
type Config struct {
	...
	ChartNameSelector  string   `split_words:"true"`                 // Run the command for multiple releases filtered by its chart's name across the cluster
	IgnoreReleases     []string `split_words:"true"`                 // Ignore releases that match with the chart selector by its name
